...
no_impeach <- filter(impeach, for_impeachment == "NO")

##I make a new dataset to more easily explore the returning results, which consist of only the member has publicly called for an impeachment inquiry
...
no_RWin2016_impeach <- filter(impeach, for_impeachment == "NO", p16winningparty == "R" )

##Same as above, but I add another filter to further refine results
...
no_RWin20162012_impeach <- filter(impeach, for_impeachment == "NO", p16winningparty == "R", p12winningparty == "R")
...
Impeach916Yes <- filter(impeach, for_impeachment == "YES", date_year == "2019", date_month == "9") 

##Used 3 filters, as it's easiest to filter both month and year rather than include every date in sepetember in the date_comb panel
...
ImpeachYesClinton <- filter(impeach, for_impeachment == "YES", clinton_percent >= 70)

##filtered, changed == to >= to properly include all datapoints
...
##created tibble to display arranged data gong from low to high
...
##used previously created dataset to more easily arrange using a tibble
...
##same as above but added filter argument, did not need to specify which dataframe bc I created new object
...
##Used previously created object to filter to only members who voted no
...
##started with impeach frame, filtered by only those prior to 2019 (did not use = sign) then arrange desc to show the highest percent at top
